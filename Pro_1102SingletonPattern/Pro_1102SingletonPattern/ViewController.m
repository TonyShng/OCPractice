...
- (void)viewDidLoad
{
    ...
    /*
     ...
     介绍
        ...
        优点: 1.在内存里只有一个实例,减少内存的开销,尤其是频繁的创建和销毁实例(比如管理学院首页缓存) 2.避免对资源的多重占用(比如写文件操作)
        缺点: 没有接口,不能集成,与单一职责原则冲突,一个类只应该只关心内部逻辑,而不关心外面怎么样来实例化
    ...
    SingletonPatternDemo *demo = [SingletonPatternDemo shareInstance];
    [demo showMessage];
